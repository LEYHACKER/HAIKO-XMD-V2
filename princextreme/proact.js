...
const sleep =  (ms) =>{
  ...
  zokou({ nomCom: "tgs", categorie: "Mods" }, async (dest, zk, commandeOptions) => {
   ...
   // if (apikey === null || apikey === 'null') { repondre('Veillez v茅rifier votre apikey ou si vous en avez pas , veiller cr茅e un compte sur api.lolhuman.xyz et vous en procurer une.'); return; };
    ...
    try {
      ...
      let msg = `   饾櫛.饾櫦.饾櫛-饾殗饾櫦饾櫝-stickers-dl
      ...
      await  repondre(msg) ;
       ...
       for ( let i = 0 ; i < (stickers.data.result.stickers).length ; i++ ) {
          ...
          let buffer = await axios({
            method: 'get',  // Utilisez 'get' pour t茅l茅charger le fichier
            url:`https://api.telegram.org/file/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/${file.data.result.file_path}` ,
            responseType: 'arraybuffer',  // D茅finissez le type de r茅ponse sur 'stream' pour g茅rer un flux de donn茅es
          })
...
zokou({ nomCom: "kickall", categorie: 'Group', reaction: "馃摚" }, async (dest, zk, commandeOptions) => {
  ...
  if (!verifGroupe) { repondre("> 鉁嬸煆� 鉁嬸煆縯his command is reserved for groups 鉂�"); return; }
